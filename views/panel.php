...
<?php if ( !empty( $cats ) ) { ?>
    <div id="<?php echo esc_attr( $ext->get_config( 'selectors/panelContainer' ) ); ?>">
        <ul class="<?php echo esc_attr( $ext->get_config( 'selectors/panelLinksWrapper' ) ); ?>">
            <?php if ( $sort_panel_type === 'ajax' ) { ?>
                <li class="active">
                    <a href="?cat=0">
                        <span> <?php esc_html_e( 'All Projects', 'ajax-portfolio' ); ?></span>
                        <?php echo $ext->get_view( 'spinner' ); ?>
                    </a>
                </li>
            <?php } ?>
