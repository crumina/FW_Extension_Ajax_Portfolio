...
<nav class="<?php echo esc_attr( $ext->get_config( 'selectors/navLinksWrapper' ) ); ?> ajax-portfolio-nav-loadmore">
    <a href="<?php echo esc_attr( "?page={$next}" ); ?>" class="<?php echo esc_attr( $ext->get_config( 'selectors/loadmoreItem' ) ); ?>">
        <span>
            <?php echo ($page < $max) ? __( 'Load more', 'ajax-portfolio' ) : __( 'Loaded all', 'ajax-portfolio' ); ?>
        </span>
